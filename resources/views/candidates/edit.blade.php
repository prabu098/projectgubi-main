                    ...
                    <div class="card-body">
                        ...
                        <form enctype="multipart/form-data" action="{{route('candidates.update',['id'=>$candidate->id])}}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="visi"  id="visi" cols="5" rows="5" class=" form-control {{ $errors->first('visi') ? "is-invalid" : ""}}">{{$candidate->visi}}</textarea>
                                <p class="text-danger">{{ $errors->first('visi') }}</p>
                            </div>
                            <div class="form-group">
                                <label for="">Misi</label>
                                <textarea name="misi"  id="misi" cols="5" rows="5" class=" form-control {{ $errors->first('misi') ? "is-invalid" : ""}}">{{$candidate->misi}}</textarea>
                                <p class="text-danger">{{ $errors->first('misi') }}</p>
                            </div>
                            <div class="form-group">
                                <label for="">Program Kerja</label>
                                <textarea name="program_kerja"  id="program_kerja" cols="5" rows="5" class=" form-control {{ $errors->first('program_kerja') ? "is-invalid" : ""}}">{{$candidate->program_kerja}}</textarea>
